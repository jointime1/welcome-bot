    ...
    try {
      ...
      const newUsers = await db
        ...
        .where(
          "created_at",
          ">=",
          sql<Date>`${yesterday.toISOString()}::timestamp`
        )
        .executeTakeFirst();
      ...
      const monthlyStats = await db
        .selectFrom("users")
        .select([
          sql<number>`EXTRACT(MONTH FROM users.created_at)`.as("month"),
          sql<number>`EXTRACT(YEAR FROM users.created_at)`.as("year"),
          ({ fn }) => fn.count("id").as("count"),
        ])
        .groupBy([
          sql<number>`EXTRACT(MONTH FROM users.created_at)`,
          sql<number>`EXTRACT(YEAR FROM users.created_at)`,
        ])
        .orderBy("year", "asc")
        .orderBy("month", "asc")
        .execute();
      ...
      const monthlyStatsFormatted = monthlyStats
        .map(
          (stat) =>
            `${monthNames[Number(stat.month) - 1]} ${stat.year}: ${
              stat.count
            } пользователей`
        )
        .join("\n");
  ...
  .text("Статистика мероприятий", async (ctx) => {
    ...
    try {
      ...
      const upcomingEvents = await db
        .selectFrom("event")
        .select(({ fn }) => [fn.count("id").as("count")])
        .where("date", ">=", sql<Date>`${now.toISOString()}::timestamp`)
        .executeTakeFirst();
      ...
      const nextEventsFormatted = nextEvents
        .map(
          (event) =>
    ...
    try {
      ...
      const newFeedback = await db
        ...
        .where(
          "created_at",
          ">=",
          sql<Date>`${lastWeek.toISOString()}::timestamp`
        )
        .executeTakeFirst();
      ...
      const recentFeedbackFormatted = recentFeedback
        .map((feedback) => {
          const username = feedback.username || feedback.user_id;
  ...
  .text("Общая статистика бота", async (ctx) => {
    ...
    try {
      ...
      const weekdayStats = await db
        .selectFrom("users")
        .select([
          sql<number>`EXTRACT(ISODOW FROM users.created_at)`.as("weekday"),
          ({ fn }) => fn.count("id").as("count"),
        ])
        .groupBy(sql`EXTRACT(ISODOW FROM users.created_at)`)
        .orderBy("weekday", "asc")
        .execute();
      ...
      const weekdayChart = weekdays
        .map(
          (day, index) =>
            `${day}: ${"▇".repeat(
              Math.ceil((weekdayData[index] / Math.max(...weekdayData, 1)) * 10)
            )} (${weekdayData[index]})`
        )
        .join("\n");
      ...
      const daysRunning = Math.floor(
        (now.getTime() - botStartDate.getTime()) / (1000 * 60 * 60 * 24)
      );
